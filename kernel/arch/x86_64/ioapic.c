...
uint32_t
ioapic_read(struct madt_ioapic *ioapic, uint8_t reg)
{
...
__attribute__((no_sanitize("undefined"))) void ioapic_write(struct madt_ioapic *ioapic, uint8_t reg, uint32_t value)
{
    uint32_t *ioapic_addr = (uint32_t *)(uintptr_t)ioapic->address;
    ioapic_addr[0] = reg;
    ioapic_addr[4] = value;
...
__attribute__((no_sanitize("undefined"))) struct madt_ioapic *ioapic_get_gsi(uint32_t gsi)
{
...
__attribute__((no_sanitize("undefined"))) void ioapic_redirect_gsi(uint32_t lapic_id, uint8_t vector, uint32_t gsi, uint16_t flags, bool mask)
{
    ...
    if (flags & (1 << 1))
        redirect |= (1 << 13); /* delivery mode */
    if (flags & (1 << 3))
        redirect |= (1 << 15); /* destination mode */
...
__attribute__((no_sanitize("undefined"))) void ioapic_redirect_irq(uint32_t lapic_id, uint8_t vector, uint8_t irq, bool mask)
{
    uint8_t index = 0;
